...
try {
  for (const [name, vp] of [["desktop", { width: 1440, height: 1000 }], ["mobile", { width: 390, height: 844 }]]) {
    ...
    assert.equal(await page.locator(".conversion-hero__copy h1").count(), 0, "headline hidden once upload starts");
    ...
    const tiles = await page.locator(".conversion-scene__row").boundingBox();
    assert(pause && card && tiles, "boxes");
    assert(pause.y >= tiles.y + tiles.height + 6, "small clear gap between MOV/MP4 tiles and the pause pill");
    assert(pause.y + pause.height > card.y && pause.y < card.y, "card overlaps the lower part of the pause pill");
    assert.equal(await page.locator(".conversion-hero__eyebrow").count(), 1, "eyebrow stays");
    assert(Math.abs(pause.x + pause.width / 2 - (vp.width / 2)) < 6, `pause centred: ${name} centre ${pause.x + pause.width / 2} vs ${vp.width / 2}`);
