...
async function tusHeadOffset(tusUploadPath: string, accessToken: string): Promise<number> {
  const res = await fetch(`${SUPABASE_URL}${tusUploadPath}`, {
    method: "HEAD",
    headers: { apikey: ANON_KEY, Authorization: `Bearer ${accessToken}`, "Tus-Resumable": "1.0.0" },
  });
  if (!res.ok) throw new Error(`Could not check upload progress: ${res.status}`);
...
export async function uploadFileResumable(
  ...
  let offset = await tusHeadOffset(tusUploadPath, session.access_token);
  ...
  while (offset < file.size) {
    ...
    if (!Number.isFinite(newOffset)) throw new Error("Storage did not report an upload offset");
    offset = newOffset;
    opts.onProgress?.(offset, file.size);
  }
}
