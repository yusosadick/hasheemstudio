  ...
  return (
    <Link to="/" aria-label="Hasheem Studio" className={cn("flex min-w-0 items-center gap-2 font-semibold tracking-tight sm:gap-3", className)}>
      <img
        ...
        alt=""
        aria-hidden="true"
        className="h-9 w-9 shrink-0 rounded-lg sm:h-12 sm:w-12"
      />
      <img
        src="/images/brand/hasheem-studio-script-wordmark.webp"
        width={640}
        height={214}
        decoding="async"
        alt="Hasheem Studio"
        className={cn("h-auto w-28 min-[400px]:w-36 min-w-0 max-w-full object-contain object-left sm:w-48", titleClassName === "text-2xl" && "sm:w-56")}
      />
    </Link>
  );
}
